...
function closePicker(result?: NotebookPickerItem): void {
  const resolve = self.notebookPickerResolve as ((item?: NotebookPickerItem) => void) | undefined
...
function button(title: string, frame: CGRect, tag: number, primary = false): UIButton {
  ...
  view.backgroundColor = primary ? UIColor.colorWithHexString("#3478F6") : UIColor.colorWithHexString("#F2F2F7")
...
function renderPicker(): void {
  ...
  const panel = new UIView({ x: Math.max(20, (bounds.width - width) / 2), y: Math.max(20, (bounds.height - height) / 2), width, height })
  panel.backgroundColor = UIColor.whiteColor()
  panel.layer.cornerRadius = 12
  panel.layer.masksToBounds = true
  overlay.addSubview(panel)
  panel.addSubview(label("绑定答案脑图", { x: 18, y: 12, width: width - 36, height: 28 }, 20, true))
  panel.addSubview(label(`请选择对应脑图 · 第 ${page + 1}/${pageCount} 页`, { x: 18, y: 40, width: width - 36, height: 22 }, 13))
  ...
  for (let row = 0; row < pageSize; row++) {
    ...
    panel.addSubview(button(`${index + 1}. ${item.title} · ${item.id.slice(-6)}`, { x: 16, y: HEADER_HEIGHT + row * ROW_HEIGHT, width: width - 32, height: 44 }, index))
  }
  const footerY = HEADER_HEIGHT + pageSize * ROW_HEIGHT + 7
  const gap = 10
  const width3 = (width - 32 - gap * 2) / 3
  panel.addSubview(button("取消", { x: 16, y: footerY, width: width3, height: 42 }, -1))
  panel.addSubview(button(page > 0 ? "上一页" : "已是首页", { x: 16 + width3 + gap, y: footerY, width: width3, height: 42 }, page > 0 ? -2 : -4))
  panel.addSubview(button(page + 1 < pageCount ? "下一页" : "已是末页", { x: 16 + (width3 + gap) * 2, y: footerY, width: width3, height: 42 }, page + 1 < pageCount ? -3 : -4, page + 1 < pageCount))
...
export function onNotebookPickerAction(sender: UIButton): void {
  ...
  if (tag === -2) { self.notebookPickerPage = Math.max(0, Number(self.notebookPickerPage) - 1); return renderPicker() }
  if (tag === -3) { self.notebookPickerPage = Number(self.notebookPickerPage) + 1; return renderPicker() }
  if (tag < 0) return
  const item = (self.notebookPickerItems as NotebookPickerItem[] | undefined)?.[tag]
  if (item) closePicker(item)
}

export function closeNotebookPicker(): void { closePicker() }
